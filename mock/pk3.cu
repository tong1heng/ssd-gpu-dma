...
__global__ void persistent_kernel1(int task_num, int task_offset, 
                                  CUDAContConcurrentQueueData<int>* task_queue, 
                                  unsigned int* stop_signal, Context* ctx,
                                  uint64_t* timer,
                                  unsigned int* iterations) {
...
__global__ void persistent_kernel2(int task_num, int task_offset, 
                                  CUDAContConcurrentQueueData<int>* task_queue, 
                                  unsigned int* stop_signal, Context* ctx,
                                  uint64_t* timer,
                                  unsigned int* iterations) {
    ...
    // while (*((volatile unsigned int*)iterations) < task_num * 3) {
    // while (*iterations < task_num * 3) {
    while (atomicAdd(iterations, 0) < task_num * 3) {
        // if (warp_idx == 0) {
        //     stop_sig = *((volatile unsigned int *)stop_signal);
        // }
        // stop_sig = __shfl_sync(0xFFFFFFFF, stop_sig, 0);
        ...
        if (threadIdx.x + threadIdx.y + threadIdx.z == 0) {
            ...
            if (pop_ok) {
                atomicAdd(iterations, 1);
                // printf("Popped task %d, total iterations: %u\n", task_id, *iterations);
            }
        }
        __syncthreads();
        if (!pop_ok) {
...
__global__ void persistent_kernel3(int task_num, int task_offset, 
                                  CUDAContConcurrentQueueData<int>* task_queue, 
                                  unsigned int* stop_signal, Context* ctx,
                                  uint64_t* timer,
                                  unsigned int* iterations) {
    ...
    // while (*iterations < task_num * 3) {
    while (true) {
        unsigned int current_iterations = atomicAdd(iterations, 0); // Read iterations atomically
        if (current_iterations >= task_num * 3) {
            break;  // Exit the loop if we have reached the desired number of iterations
        }
...
int main() {
    ...
    *persistent_io_stop_signal = 0;
    ...
    persistent_kernel3<<<BLOCKS, THREADS_PER_BLOCK, 0, stream>>>(task_num, task_offset, task_queue.d_data, persistent_io_stop_signal, ctx, timer, iterations);
